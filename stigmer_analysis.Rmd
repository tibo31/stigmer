...
```{r, echo = F}
(esp_R0 <- sum(as.numeric(names(prop_tab)) * prop_tab))
```
...
**Remark:** we can also compute the standard deviation for a round

```{r, echo = F}
esp_R0_2 <- sum(as.numeric(names(prop_tab))^2 * prop_tab)
(sd_R0 <- sqrt(esp_R0_2 - esp_R0^2)) 
```

And the standard deviation for the final gain : 
```{r, echo = F}
(sd_R0 <- sqrt(15^2*(esp_R0_2 - esp_R0^2))) 
```
...
**Remark 1:** when a player has already played the most visited cell and knows that it does not contain the value 99, it could be more interesting for him to play the 3rd most visited cell rather the 2nd one.

**Remark 2:** at the end of step 3, if a player has already played the 3 most visited cell, we have done simulations (not presented here) which show that it is more interesting to explore among the cells which has not been visited by all players. Moreover, this assumption can probably be prooved analytically.   

##### At step 4

A player visits the most visited cell if he did explore it yet. Otherwise, he tries to visit the second most visited cell. Otherwise, he tries the 3rd most visited cell. Otherwise he visits a cell which has not been explored by all the players. If they have all been explored, he chooses one of them randomly.  
...
Like in step 4. If players have already played the four first most visited cell but they do not contain 99, they should explore a cell which has not been explored yet by the community of players. If the cells have all been explored, they can choose one of the non visited randomly. 

We will simulate such a behaviour in the next section. 
...
* **exploit**: a player does not follow one of the three previous behaviours,
...
* the three worst strategy consists in copy/others/copy, exploit/copy/exploit, explore_weak/others/explore_weak. 
* the two best strategy consists in explore_strong/copy/explore_strong and explore_strong/explore/explore_weak.

####  The profile of the winners of session

We present here the profile of the 6 players who has won their session:

**Session 01, players A:**

```{r, echo = F}
filter(players_15_R1, player == "A1" & session == "session_01")
```

**Session 01, players B:**

```{r, echo = F}
filter(players_15_R1, player == "B1" & session == "session_01")
```

**Session 02, players A:**

```{r, echo = F}
filter(players_15_R1, player == "A5" & session == "session_02")
```

**Session 02, players B:**

```{r, echo = F}
filter(players_15_R1, player == "B2" & session == "session_02")
```

**Session 03, players A:**

```{r, echo = F}
filter(players_15_R1, player == "A3" & session == "session_03")
```

**Session 03, players B:**

```{r, echo = F}
filter(players_15_R1, player == "B1" & session == "session_03")
```
...
```{r}
players_15_R1 <- filter(players_15_R1, step_2 != "exploit")
```
...
We propose another strategy which consists of leaving a coin when 0 or 15 have been found. The idea is to play the cells hich have not been played until players will find the value 99. 

Let simulate such a behaviour by using the function *simu_distrib_R3()* (codes are in the .Rmd file).

```{r, echo = F}
simu_distrib_R3 <- function(game) {
  # this function simulates a party under R2 
  # where players optimize the profit for the group
  proba_cell <- matrix(1, 9, 5)
  gain <- matrix(0, 5, 5)
  ...
  while (step <= 5) {
    ...
    if (simu_k[9] == 1) 
      ...
      play_k <- c(play_k, which(simu_k == 1))
    gain[k, step] <- c(0, 0, 0, 0, 0, game, game, game, 99)[which(simu_k == 1)]
    }
   step <- step + 1
   proba_cell[play_k, ] <- 0
...
```{r, echo = F}
distrib_th_15_R3 <- NULL
for (k in 1:20000)
  distrib_th_15_R3 <- rbind(distrib_th_15_R3, simu_distrib_R3(15))
```

```{r, echo = F}
tab_15_R3 <- apply(distrib_th_15_R3, 2, table)
gather_tab_15_R3 <- data.frame(value = as.vector(tab_15_R3)/nrow(distrib_th_15_R3), 
                               gain = rep(c("0", "15", "99"), 5), 
                               step = paste0("step_", rep(1:5, each = 3))
                               )
gather_tab_15_R3$pos <- NULL
gather_tab_15_R3$pos[gather_tab_15_R3$gain == "0"] <- 0.99
gather_tab_15_R3$pos[gather_tab_15_R3$gain == "99"] <- 
  gather_tab_15_R3$value[gather_tab_15_R3$gain == "99"]/2
gather_tab_15_R3$pos[gather_tab_15_R3$gain == "15"] <- 
  0.01 + gather_tab_15_R3$value[gather_tab_15_R3$gain == "99"]
```

```{r, echo = F}
ggplot(gather_tab_15_R3, aes(y = value, x = step, fill = gain)) + 
  geom_bar(stat = "identity") + 
  geom_text(aes(x = step, y = pos, label = paste0(round(value * 100, 2) ,"%")), 
              size = 4) 
```

**Remark:** this strategy seems to give results less interesting than the previous 1. In the rest of the section, we will consider the strategy 1 as the best one.
...
### The effects th composition of the group (collaborators/liars/both) during a session
...
plot(x_lab, y_lab, xlab = "score per session", 
     ...
     col = "royalblue", xlim = c(-650, 250), ylim = c(2600, 3200))
text(x_lab, y_lab, c("both", "liar", "both", "liar", 
                     "collaborator", "liar", "liar", "liar"),
     pos = 3)
```

We observe that the more the score per session is higher, the more the average final gain per session is higher. In other terms, when a session contains too much liars, it has a negative impact on the final gain. Moreover, we remark that in the session with the highest positive score (it means a session which contains some collaborators), the winner of the session is also a collaborator. On contrary, the sessions which contain some liars have been won by liars. 

For the rest of the work, we will define the 3 sessions with the highest scores per session as "collaborative" session and the others as "non collaborative session". Indeed, we suppose that this information could have a impact on the results obtained by players. 
...
We present here the profile of the 8 players who has won their session:

**Session 01, players A:**

This a "collaborative" session. Here are the characteristics of the winner : 

```{r, echo = F}
filter(players_15_R2, player == "A5" & session == "session_01")
```

**Session 01, players B:**

This a "liar" session. Here are the characteristics of the winner : 

```{r, echo = F}
filter(players_15_R2, player == "B1" & session == "session_01")
```
...
```{r, echo = F}
filter(players_15_R2, player == "A1" & session == "session_02")
```
...
```{r, echo = F}
filter(players_15_R2, player == "B3" & session == "session_02")
```

**Session 01bis, players A:**

This a "liar" session. Here are the characteristics of the winner : 

```{r, echo = F}
filter(players_15_R2, player == "A2" & session == "session_01Bis")
```

**Session 01bis, players B:**

This a "liar" session. Here are the characteristics of the winner : 

```{r, echo = F}
filter(players_15_R2, player == "B5" & session == "session_01Bis")
```
...
```{r, echo = F}
filter(players_15_R2, player == "A1" & session == "session_03")
```
...
```{r, echo = F}
filter(players_15_R2, player == "B2" & session == "session_03")
```
...
**Interpretation:** the variable which discriminates the most the players is the one which indicates if a player is a "collaborator" or "both" ($67\%$ of the sample with a final gain equal to 2800) or bad information ($33\%$ of the sample with a final gain equal to 3331).  

* The node with the higher final gain corresponds to players who are **liar**, and who explore weakly or takes no risk at step 5.

* The node with the second highest value corresponds to players who are **liar** and belong to a "collaborative" session. 

* The node with the lowest value corresponds to players who are "collaborators" or "both" and copy at step 2. 

* The 2nd node is the node which contains the biggest number of players (12 players) who behave like this : there are players "collaborators" or "both" and "explorer" at step 2 and 5. Their final gain is the second lowest node. 

### Conclusion under R2 

* The optimal strategy presented could give the best results for the group. However, if all players had adopted such a strategy, it would have given a probability of 1/5 for players to win.

* If a players wants to increase the probability to win during a session, he has necessarly to adopt a different strategy. The theoretical example tends to show that the best situation for a player is to use the information given by the others when they are all collaborators, and do not indicate the location of the cell 99.  

* In the experiment, we can observe that players do not behave similarly depending on the session. Sometimes, players are mostly "liars", sometimes there are a mixture of "liars" and "collaborators", etc. Hence, this is very complicated to detect what could be the most interesting strategy because the profiles of session are different and what could be beter in one case is not necessarely the case in aother one. For example, in a session which contains only liars, one could think that this is preferable to explore strongly. In a session where some players indicate a good information, thus it could be better to copy. 

* Some results can be emphasized :
    * The sessions composed mostly by liars have a lower final gain than the sessions composed by collaborators.
    * The players which are liars had obtained in average a better final gain than collaborators or both
